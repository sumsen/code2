...
POWER_CHARGE_SPEED = 1000        # VOLTS / second
POWER_TRUCK_TIME = 300      # Seconds it takes the tank truck to arrive AND CONNECT TO AIRCRAFT PORTS
T_INTER = [30, 300]        # Create an aircraft every [min, max] seconds
SIM_TIME = 1000            # Simulation time in seconds


def aircraft(name, env, service_hub, fuel_pump):
    """An aircraft arrives at the gate and requests for various processes.
This process deals with power supply which is independent of other processes
including unmodeled but time-considered processes embarking and disembarking.
    There are 2 power trucks which are universal resources available to all aircrafts
    irresepective of type, airline etc
